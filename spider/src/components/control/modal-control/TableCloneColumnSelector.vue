<template>
  <div>
    <MyModal ref="modal" @ok="ok" width="80%" title="选择克隆列">
...
export default {
  ...
  computed: {
    ...
    targetColumns() {
      ...
      this.selectedTables.forEach((table) => {
        if (this.selectedColumns[table]) {
          this.selectedColumns[table].forEach((column) => {
            columns.push(this.$utils.parseJson(column));
          });
        }
      });
  ...
  methods: {
    remove(table, column) {
      let targetTable = this.selectedColumns[table];
    ...
    ok() {
      ...
      if (columns.length > 0) {
        columns.forEach((x) => {
          let column = this.getColumnInfo(x.table, x.column);
          if (column) result.push(column);
        });
        this.$emit("submit", this.table, result);
      }
